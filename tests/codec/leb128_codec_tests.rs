...
fn test_leb128_codec_reads_available_unsigned_values_unchecked() {
    ...
    let pending = unsafe { Leb128Codec::<u16, NonStrict>::decode_available_unchecked(&input, 1, 1) }
        .expect("partial unsigned value should not fail");
    assert_eq!(None, pending);

    let decoded = unsafe { Leb128Codec::<u16, NonStrict>::decode_available_unchecked(&input, 1, 2) }
        .expect("complete unsigned value should decode");
    assert_eq!(Some((300, 2)), decoded);

    let error = unsafe { Leb128Codec::<u16, Strict>::decode_available_unchecked(&[0x80, 0x00], 0, 2) }
...
fn test_leb128_codec_reads_available_signed_values_unchecked() {
    ...
    let pending = unsafe { Leb128Codec::<i16, NonStrict>::decode_available_unchecked(&input, 1, 1) }
        .expect("partial signed value should not fail");
    assert_eq!(None, pending);

    let decoded = unsafe { Leb128Codec::<i16, NonStrict>::decode_available_unchecked(&input, 1, 2) }
        .expect("complete signed value should decode");
    assert_eq!(Some((-300, 2)), decoded);

    let error = unsafe { Leb128Codec::<i16, Strict>::decode_available_unchecked(&[0xff, 0x7f], 0, 2) }
