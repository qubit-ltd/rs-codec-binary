...
fn assert_unsigned_u32_leb128_bytes(value: u32, expected: &[u8]) {
    let mut output = [0u8; Leb128Codec::<u32, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE];

    let len = unsafe { Leb128Codec::<u32, NonStrict>::encode(value, &mut output, 0) };
...
fn assert_signed_i32_leb128_bytes(value: i32, expected: &[u8]) {
    let mut output = [0u8; Leb128Codec::<i32, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE];

    let len = unsafe { Leb128Codec::<i32, NonStrict>::encode(value, &mut output, 0) };
    assert_eq!(expected.len(), len);
    assert_eq!(expected, &output[..len]);
...
fn test_leb128_codec_exposes_unit_bounds() {
    ...
    assert_eq!(10, Leb128Codec::<u64, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE);
    assert_eq!(10, Leb128Codec::<u64, NonStrict>::MAX_DECODE_UNITS_PER_VALUE);
    assert_eq!(19, Leb128Codec::<u128, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE);
    assert_eq!(19, Leb128Codec::<u128, NonStrict>::MAX_DECODE_UNITS_PER_VALUE);
    assert_eq!(
...
fn test_leb128_codec_non_strict_accepts_redundant_values() {
    let decoded = unsafe { Leb128Codec::<u16, NonStrict>::decode(&[0x80, 0x00], 0) }
        .expect("non-strict unsigned LEB128 should accept redundant zero");
    assert_decoded_eq((0, 2), decoded);

    let decoded = unsafe { Leb128Codec::<i16, NonStrict>::decode(&[0xff, 0x7f], 0) }
        .expect("non-strict signed LEB128 should accept redundant negative one");
    assert_decoded_eq((-1, 2), decoded);
}

#[test]
fn test_leb128_codec_strictly_checks_terminal_sign_extensions() {
    let decoded = unsafe { Leb128Codec::<u16, Strict>::decode(&[0x80, 0x01], 0) }
    ...
    let decoded = unsafe { Leb128Codec::<i16, Strict>::decode(&[0xff, 0x00], 0) }
        .expect("positive signed boundary value should decode");
    assert_decoded_eq((127, 2), decoded);

    let decoded = unsafe { Leb128Codec::<i16, Strict>::decode(&[0x80, 0x7f], 0) }
...
fn test_leb128_codec_roundtrips_all_u8_and_i8_values() {
    let mut unsigned_output = [0u8; Leb128Codec::<u8, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE];
    for value in u8::MIN..=u8::MAX {
        let len = unsafe { Leb128Codec::<u8, NonStrict>::encode(value, &mut unsigned_output, 0) };
        let decoded = unsafe { Leb128Codec::<u8, Strict>::decode(&unsigned_output[..len], 0) }
            .expect("canonical u8 LEB128 should decode");
        assert_decoded_eq((value, len), decoded);
    }

    let mut signed_output = [0u8; Leb128Codec::<i8, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE];
    for value in i8::MIN..=i8::MAX {
        let len = unsafe { Leb128Codec::<i8, NonStrict>::encode(value, &mut signed_output, 0) };
        let decoded = unsafe { Leb128Codec::<i8, Strict>::decode(&signed_output[..len], 0) }
...
fn test_leb128_codec_reads_and_writes_unsigned_values_unchecked() {
    let mut output = [0u8; Leb128Codec::<u16, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE + 2];
    let len = unsafe { Leb128Codec::<u16, NonStrict>::encode(300, &mut output, 1) };

    assert_eq!(2, len);
    assert_eq!([0x00, 0xac, 0x02, 0x00, 0x00], output);

    let decoded = unsafe { Leb128Codec::<u16, NonStrict>::decode(&output, 1) }.expect("valid u16 should decode");
    assert_decoded_eq((300, 2), decoded);

    let mut output = [0u8; Leb128Codec::<u16, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE];
    let len = unsafe { Leb128Codec::<u16, NonStrict>::encode(u16::MAX, &mut output, 0) };
    let decoded = unsafe { Leb128Codec::<u16, NonStrict>::decode(&output, 0) }.expect("u16::MAX should decode");
...
fn test_leb128_codec_encodes_and_decodes_through_codec_trait() {
    ...
    let written =
        unsafe { Codec::encode(&mut codec, &300, &mut output, 1) }.expect("LEB128 encoding should be infallible");
    assert_eq!(2, written);
    assert_eq!([0x00, 0xac, 0x02, 0x00, 0x00], output);

    let decoded = unsafe { Codec::decode(&mut codec, &output, 1) }.expect("valid LEB128 value should decode");
    assert_decoded_eq((300, 2), decoded);
}
...
fn test_unsigned_leb128_inherent_encode_accepts_exact_length_buffer() {
    ...
    let written = unsafe { Leb128Codec::<u64, NonStrict>::encode(0x7f, &mut output, 0) };
...
fn test_leb128_codec_trait_decodes_single_byte_unsigned_value() {
    ...
    let decoded =
        unsafe { Codec::decode(&mut codec, &input, 0) }.expect("single-byte unsigned LEB128 value should decode");
...
fn test_leb128_codec_trait_maps_decode_failures() {
    let mut codec = Leb128Codec::<u16, NonStrict>::default();
    let incomplete =
        unsafe { Codec::decode(&mut codec, &[0xac], 0) }.expect_err("partial LEB128 value should request more input");
    assert_eq!(
        DecodeFailure::incomplete_with_source(Leb128DecodeError::incomplete(0, nonzero(2), 1), nonzero(2),),
        incomplete,
    );
    assert_eq!(
...
fn test_signed_leb128_codec_encodes_and_decodes_through_codec_trait() {
    ...
    let mut output = [0u8; Leb128Codec::<i16, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE + 2];
    ...
    let decoded = unsafe { Codec::decode(&mut codec, &output, 1) }.expect("valid signed LEB128 value should decode");
    assert_decoded_eq((-300, 2), decoded);
}
...
fn test_signed_leb128_inherent_encode_accepts_exact_length_buffer() {
    ...
    let written = unsafe { Leb128Codec::<i64, NonStrict>::encode(-1, &mut output, 0) };
...
fn test_leb128_codec_trait_decodes_single_byte_signed_value() {
    ...
    let decoded =
        unsafe { Codec::decode(&mut codec, &input, 0) }.expect("single-byte signed LEB128 value should decode");
...
fn test_leb128_codec_reads_and_writes_signed_values_unchecked() {
    let mut output = [0u8; Leb128Codec::<i16, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE + 2];
    let len = unsafe { Leb128Codec::<i16, NonStrict>::encode(-300, &mut output, 1) };

    assert_eq!(2, len);
    assert_eq!([0x00, 0xd4, 0x7d, 0x00, 0x00], output);

    let decoded = unsafe { Leb128Codec::<i16, NonStrict>::decode(&output, 1) }.expect("valid i16 should decode");
    assert_decoded_eq((-300, 2), decoded);

    let mut output = [0u8; Leb128Codec::<i16, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE];
    let len = unsafe { Leb128Codec::<i16, NonStrict>::encode(300, &mut output, 0) };
    let decoded = unsafe { Leb128Codec::<i16, NonStrict>::decode(&output, 0) }.expect("positive i16 should decode");
    assert_decoded_eq((300, len), decoded);

    let mut output = [0u8; Leb128Codec::<i128, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE];
    let len = unsafe { Leb128Codec::<i128, NonStrict>::encode(i128::MIN, &mut output, 0) };
    let decoded = unsafe { Leb128Codec::<i128, NonStrict>::decode(&output, 0) }.expect("i128::MIN should decode");
    ...
    for value in values {
        output.fill(0);
        let len = unsafe { Leb128Codec::<i16, NonStrict>::encode(value, &mut output, 0) };
        let decoded =
            unsafe { Leb128Codec::<i16, NonStrict>::decode(&output, 0) }.expect("signed boundary value should decode");
        assert_decoded_eq((value, len), decoded);
    }
}
...
fn test_leb128_codec_roundtrips_all_strict_and_non_strict_instantiations() {
    macro_rules! roundtrip_unsigned {
        ($ty:ty, $value:expr) => {{
            ...
            let mut output = [0u8; Leb128Codec::<$ty, NonStrict>::MAX_ENCODE_UNITS_PER_VALUE];
            let len = unsafe { Leb128Codec::<$ty, NonStrict>::encode(value, &mut output, 0) };
            let decoded = unsafe { Leb128Codec::<$ty, NonStrict>::decode(&output, 0) }
                .expect("non-strict unsigned value should decode");
            assert_decoded_eq((value, len), decoded);

            let mut output = [0u8; Leb128Codec::<$ty, Strict>::MAX_ENCODE_UNITS_PER_VALUE];
            let len = unsafe { Leb128Codec::<$ty, Strict>::encode(value, &mut output, 0) };
            let decoded =
                unsafe { Leb128Codec::<$ty, Strict>::decode(&output, 0) }.expect("strict unsigned value should decode");
            assert_decoded_eq((value, len), decoded);
        }};
    }
    ...
    macro_rules! roundtrip_signed {
        ($ty:ty, $value:expr) => {{
            ...
            let len = unsafe { Leb128Codec::<$ty, Strict>::encode(value, &mut output, 0) };
            let decoded =
                unsafe { Leb128Codec::<$ty, Strict>::decode(&output, 0) }.expect("strict signed value should decode");
            assert_decoded_eq((value, len), decoded);
        }};
    }
...
fn test_leb128_codec_reports_incomplete_unsigned_values_unchecked() {
    ...
    let pending = unsafe { Leb128Codec::<u16, NonStrict>::decode(&input[..2], 1) }
        .expect_err("partial unsigned LEB128 should report incomplete input");
    ...
    let decoded =
        unsafe { Leb128Codec::<u16, NonStrict>::decode(&input, 1) }.expect("complete unsigned LEB128 should decode");
    assert_decoded_eq((300, 2), decoded);
...
fn test_leb128_codec_reports_incomplete_signed_values_unchecked() {
    ...
    let pending = unsafe { Leb128Codec::<i16, NonStrict>::decode(&input[..2], 1) }
    ...
    let decoded =
        unsafe { Leb128Codec::<i16, NonStrict>::decode(&input, 1) }.expect("complete signed LEB128 should decode");
    assert_decoded_eq((-300, 2), decoded);
...
fn test_leb128_codec_rejects_all_instantiated_error_paths() {
    macro_rules! reject_unsigned {
        ($ty:ty) => {{
            let unterminated = [0x80u8; Leb128Codec::<$ty, NonStrict>::MAX_DECODE_UNITS_PER_VALUE];
            let error = unsafe { Leb128Codec::<$ty, NonStrict>::decode(&unterminated, 0) }
                .expect_err("unterminated unsigned value should fail");
            assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());

            let error = unsafe { Leb128Codec::<$ty, Strict>::decode(&unterminated, 0) }
                .expect_err("unterminated strict unsigned value should fail");
            assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());

            let max_bytes = Leb128Codec::<$ty, NonStrict>::MAX_DECODE_UNITS_PER_VALUE;
            let bits = <$ty>::BITS as usize;
            let used_bits = bits - (max_bytes - 1) * 7;
            let mut malformed = [0x80u8; Leb128Codec::<$ty, NonStrict>::MAX_DECODE_UNITS_PER_VALUE];
            malformed[max_bytes - 1] = 1u8 << used_bits;
            let error = unsafe { Leb128Codec::<$ty, NonStrict>::decode(&malformed, 0) }
                .expect_err("too-wide unsigned payload should fail");
            assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());

            let mut noncanonical = [0u8; Leb128Codec::<$ty, Strict>::MAX_DECODE_UNITS_PER_VALUE];
            noncanonical[0] = 0x80;
            let error = unsafe { Leb128Codec::<$ty, Strict>::decode(&noncanonical, 0) }
                .expect_err("non-canonical unsigned value should fail");
            assert_eq!(Leb128DecodeErrorKind::NonCanonical, error.kind());
        }};
    }
...
fn test_leb128_codec_rejects_malformed_values() {
    let error = unsafe { Leb128Codec::<u16, NonStrict>::decode(&[0x80, 0x80, 0x04], 0) }
        .expect_err("too-wide unsigned payload should fail");
    assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());
    assert_eq!(0, error.start_index());
    assert_eq!(2, error.error_index());

    let error = unsafe { Leb128Codec::<u16, NonStrict>::decode(&[0x80, 0x80, 0x80], 0) }
        .expect_err("unterminated unsigned payload should fail");
    assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());
    assert_eq!(0, error.start_index());
    assert_eq!(2, error.error_index());

    let error = unsafe { Leb128Codec::<i16, NonStrict>::decode(&[0x80, 0x80, 0x04], 0) }
        .expect_err("too-wide signed payload should fail");
    assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());
    assert_eq!(0, error.start_index());
    assert_eq!(2, error.error_index());

    let error = unsafe { Leb128Codec::<i16, NonStrict>::decode(&[0x80, 0x80, 0x02], 0) }
        .expect_err("too-narrow negative signed payload should fail");
    assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());
    assert_eq!(0, error.start_index());
    assert_eq!(2, error.error_index());

    let error = unsafe { Leb128Codec::<i16, NonStrict>::decode(&[0x80, 0x80, 0x80], 0) }
...
fn test_leb128_codec_rejects_noncanonical_strict_values() {
    let decoded = unsafe { Leb128Codec::<u16, Strict>::decode(&[0xac, 0x02, 0x00], 0) }
        .expect("canonical unsigned value should decode");
    assert_decoded_eq((300, 2), decoded);

    let decoded = unsafe { Leb128Codec::<i16, Strict>::decode(&[0xd4, 0x7d, 0x00], 0) }
        .expect("canonical signed value should decode");
    assert_decoded_eq((-300, 2), decoded);

    let decoded = unsafe { Leb128Codec::<i16, Strict>::decode(&[0xac, 0x02, 0x00], 0) }
        .expect("canonical positive signed value should decode");
    assert_decoded_eq((300, 2), decoded);

    let error = unsafe { Leb128Codec::<u16, Strict>::decode(&[0x80, 0x00, 0x00], 0) }
        .expect_err("non-canonical unsigned value should fail");
    assert_eq!(Leb128DecodeErrorKind::NonCanonical, error.kind());
    assert_eq!(0, error.start_index());
    assert_eq!(1, error.error_index());

    let error = unsafe { Leb128Codec::<i16, Strict>::decode(&[0xff, 0x7f, 0x00], 0) }
        .expect_err("non-canonical signed value should fail");
    assert_eq!(Leb128DecodeErrorKind::NonCanonical, error.kind());
    assert_eq!(0, error.start_index());
    assert_eq!(1, error.error_index());
