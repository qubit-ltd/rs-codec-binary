...
fn test_leb128_codec_trait_decodes_single_byte_signed_value() {
    ...
    assert_eq!((-1, 1), decoded);
}

#[test]
fn test_leb128_codec_reads_and_writes_signed_values_unchecked() {
    let mut output = [0u8; Leb128Codec::<i16, NonStrict>::MAX_UNITS_PER_VALUE + 2];
    let len = unsafe { Leb128Codec::<i16, NonStrict>::encode_unchecked(-300, &mut output, 1) };
    ...
    for value in values {
        output.fill(0);
        let len = unsafe { Leb128Codec::<i16, NonStrict>::encode_unchecked(value, &mut output, 0) };
...
fn test_leb128_codec_reports_incomplete_unsigned_values_unchecked() {
    ...
    let pending = unsafe { Leb128Codec::<u16, NonStrict>::decode_unchecked(&input[..2], 1) }
    ...
    let decoded = unsafe { Leb128Codec::<u16, NonStrict>::decode_unchecked(&input, 1) }
        .expect("complete unsigned LEB128 should decode");
    assert_eq!((300, 2), decoded);

    let error = unsafe { Leb128Codec::<u16, Strict>::decode_unchecked(&[0x80, 0x00], 0) }
...
fn test_leb128_codec_reports_incomplete_signed_values_unchecked() {
    ...
    let pending = unsafe { Leb128Codec::<i16, NonStrict>::decode_unchecked(&input[..2], 1) }
        .expect_err("partial signed LEB128 should report incomplete input");
    assert_eq!(Leb128DecodeErrorKind::Incomplete, pending.kind());
    assert_eq!(Some(2), pending.required());
    assert_eq!(Some(1), pending.available());

    let decoded = unsafe { Leb128Codec::<i16, NonStrict>::decode_unchecked(&input, 1) }
        .expect("complete signed LEB128 should decode");
    assert_eq!((-300, 2), decoded);

    let error = unsafe { Leb128Codec::<i16, Strict>::decode_unchecked(&[0xff, 0x7f], 0) }
...
fn test_leb128_codec_rejects_all_instantiated_error_paths() {
    macro_rules! reject_unsigned {
        ($ty:ty) => {{
            ...
            let error = unsafe { Leb128Codec::<$ty, NonStrict>::decode_unchecked(&malformed, 0) }
                .expect_err("too-wide unsigned payload should fail");
            assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());

            let mut noncanonical = [0u8; Leb128Codec::<$ty, Strict>::MAX_UNITS_PER_VALUE];
            noncanonical[0] = 0x80;
            let error = unsafe { Leb128Codec::<$ty, Strict>::decode_unchecked(&noncanonical, 0) }
                .expect_err("non-canonical unsigned value should fail");
            assert_eq!(Leb128DecodeErrorKind::NonCanonical, error.kind());
        }};
    }
...
fn test_leb128_codec_rejects_malformed_values() {
    let error = unsafe { Leb128Codec::<u16, NonStrict>::decode_unchecked(&[0x80, 0x80, 0x04], 0) }
        .expect_err("too-wide unsigned payload should fail");
    assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());
    assert_eq!(2, error.index());

    let error = unsafe { Leb128Codec::<u16, NonStrict>::decode_unchecked(&[0x80, 0x80, 0x80], 0) }
        .expect_err("unterminated unsigned payload should fail");
    assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());
    assert_eq!(2, error.index());

    let error = unsafe { Leb128Codec::<i16, NonStrict>::decode_unchecked(&[0x80, 0x80, 0x04], 0) }
        .expect_err("too-wide signed payload should fail");
    assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());
    assert_eq!(2, error.index());

    let error = unsafe { Leb128Codec::<i16, NonStrict>::decode_unchecked(&[0x80, 0x80, 0x02], 0) }
        .expect_err("too-narrow negative signed payload should fail");
    assert_eq!(Leb128DecodeErrorKind::Malformed, error.kind());
    assert_eq!(2, error.index());

    let error = unsafe { Leb128Codec::<i16, NonStrict>::decode_unchecked(&[0x80, 0x80, 0x80], 0) }
...
fn test_leb128_codec_rejects_noncanonical_strict_values() {
    let decoded = unsafe { Leb128Codec::<u16, Strict>::decode_unchecked(&[0xac, 0x02, 0x00], 0) }
        .expect("canonical unsigned value should decode");
    assert_eq!((300, 2), decoded);

    let decoded = unsafe { Leb128Codec::<i16, Strict>::decode_unchecked(&[0xd4, 0x7d, 0x00], 0) }
        .expect("canonical signed value should decode");
    assert_eq!((-300, 2), decoded);

    let decoded = unsafe { Leb128Codec::<i16, Strict>::decode_unchecked(&[0xac, 0x02, 0x00], 0) }
        .expect("canonical positive signed value should decode");
    assert_eq!((300, 2), decoded);

    let error = unsafe { Leb128Codec::<u16, Strict>::decode_unchecked(&[0x80, 0x00, 0x00], 0) }
        .expect_err("non-canonical unsigned value should fail");
    assert_eq!(Leb128DecodeErrorKind::NonCanonical, error.kind());
    assert_eq!(0, error.index());

    let error = unsafe { Leb128Codec::<i16, Strict>::decode_unchecked(&[0xff, 0x7f, 0x00], 0) }
        .expect_err("non-canonical signed value should fail");
    assert_eq!(Leb128DecodeErrorKind::NonCanonical, error.kind());
    assert_eq!(0, error.index());
}
