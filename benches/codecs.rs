...
fn unsigned_values(distribution: Distribution) -> Vec<u64> {
    match distribution {
        ...
        Distribution::Short => {
            ...
            (0..BATCH_SIZE).map(|_| next_state(&mut state) & 0x3fff).collect()
        }
        Distribution::Uniform => {
            let mut state = 0xD1CE_BA5E_1234_5678;
...
fn signed_leb128_values(distribution: Distribution) -> Vec<i64> {
    match distribution {
        ...
        Distribution::Uniform => {
            let mut state = 0xA11C_E5E1_5EED_1234;
            (0..BATCH_SIZE).map(|_| next_state(&mut state) as i64).collect()
        }
        Distribution::MaxWidth => {
            let mut state = 0xA11C_E5E1_5EED_1234;
            (0..BATCH_SIZE)
...
fn zig_zag_values(distribution: Distribution) -> Vec<i64> {
    match distribution {
        Distribution::Boundary => {
            ...
            let pattern = encoded.into_iter().map(decode_zig_zag_value).collect::<Vec<_>>();
            repeat_pattern(&pattern)
        }
        Distribution::Short => {
...
fn validate_distribution_widths(distribution: Distribution, widths: impl IntoIterator<Item = usize>) {
    match distribution {
        Distribution::Boundary => {
    ...
    P: Leb128DecodePolicy,
{
    ...
    for payload in black_box(payloads) {
        ...
        let (value, consumed) =
            unsafe { Leb128Codec::<u64, P>::decode(&payload.bytes[..payload.len], 0).unwrap_unchecked() };
    ...
    P: Leb128DecodePolicy,
{
    ...
    for payload in black_box(payloads) {
        ...
        // payloads produced by the corresponding encoder.
        let (value, consumed) =
            unsafe { Leb128Codec::<i64, P>::decode(&payload.bytes[..payload.len], 0).unwrap_unchecked() };
    ...
    P: Leb128DecodePolicy,
{
    ...
    for payload in black_box(payloads) {
        ...
        // payloads produced by the corresponding encoder.
        let (value, consumed) =
            unsafe { ZigZagCodec::<i64, P>::decode(&payload.bytes[..payload.len], 0).unwrap_unchecked() };
        checksum = mix_checksum(checksum, value as u64);
        checksum = mix_checksum(checksum, consumed.get() as u64);
    }
    checksum
...
fn checksum_decode_error(checksum: u64, error: &Leb128DecodeError) -> u64 {
    ...
    checksum = mix_checksum(checksum, error.consumed().map_or(0, |count| count.get() as u64));
    mix_checksum(checksum, error.required().map_or(0, |count| count.get() as u64))
}
    ...
    for field in fields {
        ...
        match field {
            MixedBinaryField::U8(value) => {
                let written = unsafe { BinaryCodec::<u8, BigEndian>::encode(*value, output, output_index) };
                debug_assert_eq!(written, width);
            }
            MixedBinaryField::U16(value) => {
                let written = unsafe { BinaryCodec::<u16, BigEndian>::encode(*value, output, output_index) };
                debug_assert_eq!(written, width);
            }
            MixedBinaryField::U32(value) => {
                let written = unsafe { BinaryCodec::<u32, BigEndian>::encode(*value, output, output_index) };
                debug_assert_eq!(written, width);
            }
            MixedBinaryField::U64(value) => {
                let written = unsafe { BinaryCodec::<u64, BigEndian>::encode(*value, output, output_index) };
                debug_assert_eq!(written, width);
            }
            MixedBinaryField::U128(value) => {
                let written = unsafe { BinaryCodec::<u128, BigEndian>::encode(*value, output, output_index) };
                debug_assert_eq!(written, width);
            }
        }
        offset += width;
    ...
    for field in fields {
        ...
        match field {
            MixedBinaryField::U8(_) => {
                let (value, consumed) = unsafe { BinaryCodec::<u8, BigEndian>::decode(input, input_index) };
                debug_assert_eq!(consumed.get(), width);
                checksum = mix_checksum(checksum, u64::from(value));
            }
            MixedBinaryField::U16(_) => {
                let (value, consumed) = unsafe { BinaryCodec::<u16, BigEndian>::decode(input, input_index) };
                debug_assert_eq!(consumed.get(), width);
                checksum = mix_checksum(checksum, u64::from(value));
            }
            MixedBinaryField::U32(_) => {
                let (value, consumed) = unsafe { BinaryCodec::<u32, BigEndian>::decode(input, input_index) };
                debug_assert_eq!(consumed.get(), width);
                checksum = mix_checksum(checksum, u64::from(value));
            }
            MixedBinaryField::U64(_) => {
                let (value, consumed) = unsafe { BinaryCodec::<u64, BigEndian>::decode(input, input_index) };
                debug_assert_eq!(consumed.get(), width);
                checksum = mix_checksum(checksum, value);
            }
            MixedBinaryField::U128(_) => {
                let (value, consumed) = unsafe { BinaryCodec::<u128, BigEndian>::decode(input, input_index) };
                debug_assert_eq!(consumed.get(), width);
                checksum = mix_checksum(checksum, value as u64);
            }
        }
...
fn decode_mixed_binary_safe_slices(fields: &[MixedBinaryField], input: &[u8]) -> u64 {
    ...
    for field in fields {
        ...
        match field {
            ...
            MixedBinaryField::U128(_) => {
                let (value, consumed) = unsafe { BinaryCodec::<u128, BigEndian>::decode(window, 0) };
                debug_assert_eq!(consumed.get(), width);
                checksum = mix_checksum(checksum, value as u64);
            }
        }
...
fn build_mixed_uleb_fixture() -> MixedUlebFixture {
    ...
    assert!(widths.iter().all(|&width| (1..=MAX_MIXED_ULEB_BYTES).contains(&width)));
...
fn mixed_uleb_width(field: &MixedUlebField) -> usize {
    match field {
        ...
        MixedUlebField::Usize(value) => Leb128Codec::<usize, NonStrict>::default().encode_len(value),
...
fn encode_mixed_uleb_unchecked(fields: &[MixedUlebField], widths: &[usize], output: &mut [u8]) -> u64 {
    ...
    for index in 0..fields.len() {
        ...
        let written = match field {
            MixedUlebField::U8(value) => unsafe { Leb128Codec::<u8, NonStrict>::encode(*value, output, output_index) },
...
fn encode_mixed_uleb_safe_slices(fields: &[MixedUlebField], widths: &[usize], output: &mut [u8]) -> u64 {
    debug_assert_eq!(fields.len(), widths.len());
    let fields = black_box(fields);
    let widths = black_box(widths);
    let mut offset = 0_usize;
...
fn decode_mixed_uleb_unchecked(fields: &[MixedUlebField], input: &[u8]) -> u64 {
    ...
    for field in fields {
        ...
        match field {
            MixedUlebField::U8(_) => {
                let (value, consumed) =
                    unsafe { Leb128Codec::<u8, NonStrict>::decode(input, input_index).unwrap_unchecked() };
                checksum = mix_checksum(checksum, u64::from(value));
                offset += consumed.get();
            }
            MixedUlebField::U16(_) => {
                let (value, consumed) =
                    unsafe { Leb128Codec::<u16, NonStrict>::decode(input, input_index).unwrap_unchecked() };
                checksum = mix_checksum(checksum, u64::from(value));
                offset += consumed.get();
            }
            MixedUlebField::U32(_) => {
                let (value, consumed) =
                    unsafe { Leb128Codec::<u32, NonStrict>::decode(input, input_index).unwrap_unchecked() };
                checksum = mix_checksum(checksum, u64::from(value));
                offset += consumed.get();
            }
            MixedUlebField::U64(_) => {
                let (value, consumed) =
                    unsafe { Leb128Codec::<u64, NonStrict>::decode(input, input_index).unwrap_unchecked() };
                checksum = mix_checksum(checksum, value);
                offset += consumed.get();
            }
            MixedUlebField::Usize(_) => {
                let (value, consumed) =
                    unsafe { Leb128Codec::<usize, NonStrict>::decode(input, input_index).unwrap_unchecked() };
                checksum = mix_checksum(checksum, value as u64);
                offset += consumed.get();
            }
            MixedUlebField::U128(_) => {
                let (value, consumed) =
                    unsafe { Leb128Codec::<u128, NonStrict>::decode(input, input_index).unwrap_unchecked() };
                checksum = mix_checksum(checksum, value as u64);
                offset += consumed.get();
            }
        }
...
fn decode_mixed_uleb_safe_slices(fields: &[MixedUlebField], input: &[u8]) -> u64 {
    ...
    for field in fields {
        ...
        match field {
            MixedUlebField::U8(_) => {
                let (value, consumed) = unsafe { Leb128Codec::<u8, NonStrict>::decode(window, 0).unwrap_unchecked() };
                checksum = mix_checksum(checksum, u64::from(value));
                offset += consumed.get();
            }
            MixedUlebField::U16(_) => {
                let (value, consumed) = unsafe { Leb128Codec::<u16, NonStrict>::decode(window, 0).unwrap_unchecked() };
                checksum = mix_checksum(checksum, u64::from(value));
                offset += consumed.get();
            }
            MixedUlebField::U32(_) => {
                let (value, consumed) = unsafe { Leb128Codec::<u32, NonStrict>::decode(window, 0).unwrap_unchecked() };
                checksum = mix_checksum(checksum, u64::from(value));
                offset += consumed.get();
            }
            MixedUlebField::U64(_) => {
                let (value, consumed) = unsafe { Leb128Codec::<u64, NonStrict>::decode(window, 0).unwrap_unchecked() };
                checksum = mix_checksum(checksum, value);
                offset += consumed.get();
            }
            MixedUlebField::Usize(_) => {
                let (value, consumed) =
                    unsafe { Leb128Codec::<usize, NonStrict>::decode(window, 0).unwrap_unchecked() };
                checksum = mix_checksum(checksum, value as u64);
                offset += consumed.get();
            }
            MixedUlebField::U128(_) => {
                let (value, consumed) = unsafe { Leb128Codec::<u128, NonStrict>::decode(window, 0).unwrap_unchecked() };
                checksum = mix_checksum(checksum, value as u64);
                offset += consumed.get();
            }
        }
...
fn bench_mixed_uleb(c: &mut Criterion) {
    ...
    let mut unchecked_encoded = vec![0_u8; storage_len];
    let mut safe_encoded = vec![0_u8; storage_len];
    let unchecked_checksum = encode_mixed_uleb_unchecked(&fixture.fields, &fixture.widths, &mut unchecked_encoded);
    let safe_checksum = encode_mixed_uleb_safe_slices(&fixture.fields, &fixture.widths, &mut safe_encoded);
    assert_eq!(unchecked_encoded, safe_encoded);
    assert_eq!(unchecked_checksum, safe_checksum);
    ...
    group.bench_function("decode_unchecked", |bencher| {
        bencher.iter(|| black_box(decode_mixed_uleb_unchecked(&fixture.fields, &fixture.encoded)));
    });
    group.bench_function("decode_safe_slices", |bencher| {
        bencher.iter(|| black_box(decode_mixed_uleb_safe_slices(&fixture.fields, &fixture.encoded)));
    });
    group.finish();
}
...
fn bench_leb128(c: &mut Criterion) {
    ...
    for distribution in DISTRIBUTIONS {
        ...
        group.bench_with_input(BenchmarkId::new("u64_encode_len", name), &values, |bencher, values| {
        ...
        group.bench_with_input(
            ...
            &values,
            |bencher, values| {
                bencher.iter(|| black_box(encode_exact_capacity::<Leb128Codec<u64, NonStrict>>(values)));
            },
        );
        group.bench_with_input(
        ...
        group.bench_with_input(BenchmarkId::new("i64_encode_len", name), &values, |bencher, values| {
        ...
        group.bench_with_input(
            ...
            &values,
            |bencher, values| {
                bencher.iter(|| black_box(encode_exact_capacity::<Leb128Codec<i64, NonStrict>>(values)));
            },
        );
        group.bench_with_input(
...
fn bench_zig_zag(c: &mut Criterion) {
    ...
    for distribution in DISTRIBUTIONS {
        ...
        group.bench_with_input(
            ...
            &values,
            |bencher, values| {
                bencher.iter(|| black_box(encode_exact_capacity::<ZigZagCodec<i64, NonStrict>>(values)));
            },
        );
        group.bench_with_input(
            BenchmarkId::new("i64_decode_non_strict", name),
